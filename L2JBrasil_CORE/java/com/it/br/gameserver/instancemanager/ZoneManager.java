...
public class ZoneManager {
    ...
    private final void load() {
        ...
        try {
            ...
            if (!file.exists()) {
                _log.error("The zone.xml file {} is missing.", file.getAbsolutePath());
                return;
            }
                ...
                if ("list".equalsIgnoreCase(n.getNodeName())) {
                    for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling()) {
                        if ("zone".equalsIgnoreCase(d.getNodeName())) {
                            ...
                            if (temp == null) {
                                _log.warn("ZoneManager: No such zone type: {}", zoneType);
                                continue;
                            }
                            ...
                            try {
                                ...
                                if (zoneShape.equals("Cuboid")) {
                                    ...
                                    for (int i = 0; i < 2; i++) {
                                        if (rset.next()) {
                                            ...
                                            _log.warn("ZoneManager: Missing cuboid vertex in sql data for zone: {}", zoneId);
                                            rset.close();
                                            statement.close();
                                    ...
                                    if ((fl_x.size() == fl_y.size()) && (fl_x.size() > 2)) {
                                        ...
                                        _log.warn("ZoneManager: Bad sql data for zone: {}", zoneId);
                                        rset.close();
                                        statement.close();
                                        continue;
                                    }
                                } else {
                                    _log.warn("ZoneManager: Unknown shape: {}", zoneShape);
                                    rset.close();
                                    statement.close();
                                    continue;
                                }
                                ...
                                _log.warn("ZoneManager: Failed to load zone coordinates", e);
                            }
