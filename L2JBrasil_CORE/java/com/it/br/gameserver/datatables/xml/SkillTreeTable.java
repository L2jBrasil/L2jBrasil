...
public class SkillTreeTable {
    ...
    private SkillTreeTable() {
        ...
        if (!file.exists()) {
            _log.warn("Could not be loaded: file {} not found", file.getAbsolutePath());
            return;
        }
        try {
        ...
        if (!f2.exists()) {
            _log.warn("could not be loaded: file {} not found", f2.getAbsolutePath());
            return;
        }
        try {
...
//									String type = String.valueOf(t2.getAttributes().getNamedItem("type").getNodeValue());
        ...
        if (!f1.exists()) {
            _log.warn("Could not be loaded: file {} not found", f1.getAbsolutePath());
            return;
        }
        try {
        ...
        _log.info("FishingSkillTreeTable: Loaded {} general skills.", count2);
        _log.info("DwarvenSkillTreeTable: Loaded {} dwarven skills.", count3);
        _log.info("EnchantSkillTreeTable: Loaded {} enchant skills.", count6);
        _log.info("EnchantSkillTreeTable: Loaded {} enchant stats.", count4);
        _log.info("PledgeSkillTreeTable: Loaded {} pledge skills.", count5);
    }
    ...
    public int getMinLevelForNewSkill(L2PcInstance cha, ClassId classId) {
        ...
        if (skills == null) {
            // the skilltree for this class is undefined, so we give an empty list
            _log.warn("Skilltree for class {} is not defined!", classId);
            return minLevel;
        }
