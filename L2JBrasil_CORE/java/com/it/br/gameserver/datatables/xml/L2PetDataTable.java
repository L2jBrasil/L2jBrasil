...
public class L2PetDataTable {
    ...
    public void loadPetsData() {
        ...
        File f = new File(serverSettings.getDatapackDirectory() + "/data/xml/pet_stats.xml");
        if (!f.exists()) {
            _log.warn("pet_stats.xml could not be loaded: file {} not found", f.getAbsolutePath());
            return;
        }
        int k = 0;
        try {
        ...
        _log.info("PetStatsTable: Loaded {} pets with {} stats.", _petTable.size(), k);
    }
