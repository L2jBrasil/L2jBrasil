...
public class NpcTable {
    ...
    private static void loadSKillLearnFromXml() {
        ...
        if (!f.exists()) {
            _log.error("Could not be loaded: file {} not found", f.getAbsolutePath());
            return;
        }
        try {
        ...
        _log.info("NpcTable: Loaded {} teachers.", th);
    }
    ...
    private static void loadMinionsFromXml() {
        ...
        if (!f1.exists()) {
            _log.error("minion.xml could not be loaded: file {} not found", f1.getAbsolutePath());
            return;
        }
        try {
